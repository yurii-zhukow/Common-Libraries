...
namespace YZ {
    public static partial class Helpers {
        ...
        public static T Nearest<T>( this IEnumerable<T> src, T value, Func<T, T, decimal> distance, [CallerArgumentExpression( nameof( src ) )] string expr = "" ) {
            if ( !src?.Any() ?? false ) throw new ArgumentException( $"{expr} is empty.", expr );
            if ( src.Count() == 1 ) return src.First();
            var r = src.First();
            var d = distance(r, value);
            return src.Skip( 1 ).Aggregate( (r, d), ( acc, v ) => { var d = distance(v, value); return d < acc.d ? (v, d) : acc; } ).r;
        }
        public static T Nearest<T>( this IEnumerable<T> src, T value, Func<T, T, uint> distance, [CallerArgumentExpression( nameof( src ) )] string expr = "" ) {
            if ( !src?.Any() ?? false ) throw new ArgumentException( $"{expr} is empty.", expr );
            if ( src.Count() == 1 ) return src.First();
            var r = src.First();
            var d = distance(r, value);
        ...
        public static T Nearest<T>( this IEnumerable<T> src, Func<T, decimal> distance, [CallerArgumentExpression( nameof( src ) )] string expr = "" ) {
            if ( !src?.Any() ?? false ) throw new ArgumentException( $"{expr} is empty.", expr );
            if ( src.Count() == 1 ) return src.First();
            var r = src.First();
            var d = distance(r);
            return src.Skip( 1 ).Aggregate( (r, d), ( acc, v ) => { var d = distance(v); return d < acc.d ? (v, d) : acc; } ).r;
        }
        public static T Nearest<T>( this IEnumerable<T> src, Func<T, uint> distance, [CallerArgumentExpression( nameof( src ) )] string expr = "" ) {
            if ( !src?.Any() ?? false ) throw new ArgumentException( $"{expr} is empty.", expr );
            if ( src.Count() == 1 ) return src.First();
            var r = src.First();
            var d = distance(r);
        ...
        public static int NearestIndex<T, TProp, TDiff>( this T[] src, TProp value, Func<T, TProp> fld, Func<TProp, TProp, TDiff> diff ) where TProp : IComparable<TProp> where TDiff : IComparable<TDiff> {

            var l = src?.Length ?? 0;
            if ( l == 0 ) return -1;
            if ( value.CompareTo( fld( src[ 0 ] ) ) <= 0 ) return 0;
            if ( value.CompareTo( fld( src[ l - 1 ] ) ) >= 0 ) return l - 1;
            ...
            // lo == hi + 1
            var loDiff = diff(fld(src[lo]), value);
            var hiDiff = diff(value, fld(src[hi]));

            return ( loDiff.CompareTo( hiDiff ) < 0 ) ? lo : hi;
        }

        public static (int ix, T value) Nearest<T, TProp, TDiff>( this T[] src, TProp value, Func<T, TProp> fld, Func<TProp, TProp, TDiff> diff ) where TProp : IComparable<TProp> where TDiff : IComparable<TDiff> {
            var ix = NearestIndex(src, value, fld, diff);
            return (ix, ix >= 0 ? src[ ix ] : default);
        }
